...
class TestQueryEngine(unittest.TestCase):
    ...
    def test_time_spent_breakdown_for_tasks(self):
        """Time spent query should include per-activity breakdown"""
        now = datetime.now()
        samples = [
            {
        ...
        for event in samples:
            self.engine.store_context(event)

        result = self.engine.answer_query("How much time spent on task today?")

        self.assertEqual(result["type"], "time_spent")
        answer = result["answer"]
        self.assertEqual(answer["category"], "TASK")
        self.assertIn("by_activity", answer)
        self.assertTrue(
            any(item["activity"] == "PROGRAMMING" for item in answer["by_activity"])
        )
        self.assertTrue(
            any(item["activity"] == "EMAILS" for item in answer["by_activity"])
        )
...
class TestArchitectureInvariants(unittest.TestCase):
    ...
    def test_inference_pure_function(self):
        ...
        if sessions1 and sessions2:
            self.assertEqual(sessions1[0]["category"], sessions2[0]["category"])
